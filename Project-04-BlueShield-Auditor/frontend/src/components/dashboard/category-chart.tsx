...
import type { CategoryFailureCount } from "@/types/api"

interface CategoryChartProps {
  data: CategoryFailureCount[]
}
...
export function CategoryChart({ data }: CategoryChartProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Most Common Failing Categories</CardTitle>
      </CardHeader>
      <CardContent className="h-64 pt-2">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 16, left: 0, bottom: 0 }}>
            ...
            />
            <Bar dataKey="count" fill="var(--color-fair)" radius={[0, 4, 4, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
